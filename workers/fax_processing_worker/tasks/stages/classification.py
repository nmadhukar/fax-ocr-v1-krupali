...
def match_template(ctx: PipelineContext) -> None:
    ...
    ctx.match_result = ctx.template_matcher.match_best_page(
...
def apply_page_rotation(ctx: PipelineContext) -> None:
    ...
    for page_num_str, angle in rotate_config.items():
        ...
        rotated = _rotate_image(ctx.pages[page_num - 1], int(angle))
        if rotated is ctx.pages[page_num - 1]:
            continue
    ...
    if rotated_any:
        # Keep downstream classification/scanners consistent with rotated OCR.
